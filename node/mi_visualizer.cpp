...
class MutualInformationVisualizer {
  ...
  public:
    ...
    MutualInformationVisualizer() {
      ...
      std::string mi_topic, p_not_measured_topic, mi_points_topic;
    ...
    void map_callback(const nav_msgs::OccupancyGrid & map_msg) {
      ...
      while (ros::ok()) {
        ...
        point.x = x * map_info.resolution;
        point.y = y * map_info.resolution;
        mi_points_msg.points.push_back(point);

        // Condition on the maximum point
        w.condition(x, y, condition_steps);
      }
    }
  ...
  private:
    ...
    ros::Publisher mi_pub, p_not_measured_pub, mi_points_pub;
